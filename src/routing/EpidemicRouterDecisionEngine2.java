 ...
 */
public class EpidemicRouterDecisionEngine2 implements RoutingDecisionEngine {
    ...
    public EpidemicRouterDecisionEngine2(Settings s) {

    }

    public EpidemicRouterDecisionEngine2(EpidemicRouterDecisionEngine2 prototype) {
    }
    ...
    public boolean shouldSendMessageToHost(Message m, DTNHost otherHost) {
        m.updateProperty("value", getMarginalUtility(m, getHost()));
        return true;
    }
    ...
    private EpidemicRouterDecisionEngine2 getOtherEpidemicRouter2(DTNHost host) {
        MessageRouter otherRouter = host.getRouter();
        assert otherRouter instanceof DecisionEngineRouter : "This router only works "
                + " with other routers of same type";

        return (EpidemicRouterDecisionEngine2) ((DecisionEngineRouter) otherRouter).getDecisionEngine();
    }

    @Override
    public RoutingDecisionEngine replicate() {
        return new EpidemicRouterDecisionEngine2(this);
    }
...
//        MAXIMUM_DELAY;
    ...
    private double getMarginalUtility(Message msg, DTNHost host) {
        ...
        final EpidemicRouterDecisionEngine2 otherRouter = getOtherEpidemicRouter2(host);

        return getMarginalUtility(msg, otherRouter, host);
    }

    private double getMarginalUtility(Message msg, EpidemicRouterDecisionEngine2 router, DTNHost host) {
    ...
    private double computeRemainingTime(Message msg) {
        ...
        if (delayTable.getDelayEntryByMessageId(msg.getId()) != null) {
            ...
            for (Map.Entry<DTNHost, Tuple<Double, Double>> entry : delayTable.getDelayEntryByMessageId(msg.getId()).getDelays()) {
                ...
                transferTime = getOtherEpidemicRouter2(host).computeTransferTime(msg, msg.getTo());
